...
class Wheel {
	...
	constructor(world, scene, carBody, x: number, y: number, z: number) {
		...
		this.mesh = this.createWheelMesh(scene, wheelRadius, wheelWidth, material)
		...
		frameA.setIdentity();
		frameA.setOrigin(new Ammo.btVector3(0, -Math.sign(x) * 0.5, 0));
		...
		for (let i = 0; i < 6; ++i) constraint.setParam(1, erp, i)
		for (let i = 0; i < 6; ++i) constraint.setParam(2, erp, i)
		for (let i = 0; i < 6; ++i) constraint.setParam(3, 0, i)
		for (let i = 0; i < 6; ++i) constraint.setParam(4, 0, i)
	...
	private createWheelMesh(scene, radius, width, material) {
		...
		// mesh.add(new THREE.Mesh(new THREE.BoxGeometry(width * 1.5, radius * 1.2, radius*.25, 1, 1, 1), material));
		scene.add(mesh);
